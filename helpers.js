...
window.helpers = {
    uniques: function (data, node) {
        var uniques = [];

        data.forEach(function (d) {
            if (uniques.indexOf(node(d)) < 0) {
                uniques.push(node(d));
            }
        });
    ...
    node_id: function (data, node) {
        var uniques = helpers.uniques(data, node);
    ...
    tooltip: function (text) {
        ...
        return function (selection) {
            ...
            function mouseover(d) {
                ...
                tool.append('rect')
                    .attrs({height: textNode.getBBox().height,
    ...
    connection_matrix: function (data) {
        var nick_id = helpers.nick_id(data, function (d) { return [d.source, d.target]; }),
        ...
        data.forEach(function (d) {
            matrix[nick_id(d.source)][nick_id(d.target)] += 1;
        });
